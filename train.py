...
def get_total_queue_length(lanes):
    queue_length = 0
    for lane in lanes:
...
class Agent:
    ...
    def learn(self, junction):
        ...
        # use all stored samples for this junction (you can change to random sample if desired)
        batch = np.arange(mem_count, dtype=np.int32)

        state_batch = torch.tensor(self.memory[junction]["state_memory"][batch], dtype=torch.float).to(
            self.Q_eval.device
        )
        new_state_batch = torch.tensor(
            self.memory[junction]["new_state_memory"][batch], dtype=torch.float
        ).to(self.Q_eval.device)
        reward_batch = torch.tensor(
            self.memory[junction]['reward_memory'][batch], dtype=torch.float
        ).to(self.Q_eval.device)
        terminal_batch = torch.tensor(self.memory[junction]['terminal_memory'][batch], dtype=torch.bool).to(self.Q_eval.device)
        action_batch = torch.tensor(self.memory[junction]["action_memory"][batch], dtype=torch.long).to(self.Q_eval.device)

        # q_eval: predicted Q-values for current states, then pick the Q for taken actions
        q_eval_all = self.Q_eval.forward(state_batch)  # shape: (N, n_actions)
        q_eval = q_eval_all.gather(1, action_batch.unsqueeze(1)).squeeze(1)  # shape: (N,)

        # q_next: predicted Q-values for next states
        q_next_all = self.Q_eval.forward(new_state_batch)
        # zero out next Q for terminal states
        q_next_all[terminal_batch] = 0.0
        q_next_max, _ = torch.max(q_next_all, dim=1)

        q_target = reward_batch + self.gamma * q_next_max

        # MSELoss expects (input(prediction), target)
...
def run(train=True,model_name="model",epochs=200,steps=500,ard=False):
    if ard:
    ...
    # start SUMO briefly to read junctions, then close
    traci.start(
    ...
    brain = Agent(
        gamma=0.99,
        epsilon=0.1,  # small exploration to start training (if training)
        lr=0.1,
        input_dims=4,
    ...
    # if running in test mode but model is missing, skip loading (don't crash)
    ...
    if not train:
        if os.path.exists(model_path):
            ...
            print(f"Warning: model file '{model_path}' not found. Continuing without loading (running without pretrained weights).")

    print("device:", brain.Q_eval.device)
    ...
    for e in range(epochs):
        ...
        while step <= steps:
            ...
            for junction_number, junction in enumerate(all_junctions):
                ...
                queue_length = get_total_queue_length(controled_lanes)
                total_time += waiting_time
                total_queue_length += queue_length
                total_vehicles_passed += get_total_vehicles_passed()
